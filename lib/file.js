...
var getBlocks = function (content) {
  ...
  var parser = new htmlparser.Parser({
    onopentag: function (name, attribs) {
      if ((name === 'script' || name === 'link') && block && last) {

        if (attribs) {
          if (attribs.src && !!attribs.src) {
            asset = attribs.src;
          } else if (attribs.href && !!attribs.href) {
            asset = attribs.href;
          }
        }
        ...
        if (asset) {
          ...
          if (attribs && attribs.media && !!attribs.media) {
            last.media = attribs.media;
          }

          // preserve defer attribute
          var defer = attribs && attribs.defer;
          ...
          var async = attribs && attribs.async;
          ...
          var main = attribs && attribs['data-main'];
          if (main) {
            throw new Error('require.js blocks are no more supported.');
          }
        }
        //last.raw.push(node.data);
        asset = null;
      }
    },
    oncomment: function (data) {

      var indent = (data.match(/^\s*/) || [])[0];
      var endbuild = regend.test(data);
      var build = data.match(regbuild);
      ...
      var isConditionalStart = data.match(/(\[if.*\]>)(<!-->)?( -->)?/g);
      var isConditionalEnd = data.match(/(<!--\s?)?(<!\[endif\])/g);
